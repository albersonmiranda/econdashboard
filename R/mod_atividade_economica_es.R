          ...
          footer = fluidRow(
            column(
              ...
              descriptionBlock(
                ...
                numberColor = if(tail(series$PIBES$value,1)-head(tail(series$PIBES$value, 2), 1) >= 0) {"green"} else {"red"}, 
                numberIcon = if(tail(series$PIBES$value,1)-head(tail(series$PIBES$value, 2), 1) >= 0) {"fas fa-caret-up"} else {"fas fa-caret-down"},
        ...
        boxPlus(
          ...
          footer = fluidRow(
            column(
              ...
              descriptionBlock(
                number = paste(round(
                  tail(series$IBCRES$value, 1) - head(tail(series$IBCRES$value, 2), 1), 2), "pts"), 
                numberColor = if(tail(series$IBCRES$value, 1) - head(tail(series$IBCRES$value, 2), 1) >= 0) {"green"} else {"red"}, 
                numberIcon = if(tail(series$IBCRES$value, 1) - head(tail(series$IBCRES$value, 2), 1) >= 0) {"fas fa-caret-up"} else {"fas fa-caret-down"},
              ...
              descriptionBlock(
                number = paste(round(
                  tail(series$IBCRESs$value,1)-head(tail(series$IBCRESs$value, 2), 1), 2), "pts"), 
                numberColor = if(tail(series$IBCRESs$value,1)-head(tail(series$IBCRESs$value, 2), 1) >= 0) {"green"} else {"red"}, 
                numberIcon = if(tail(series$IBCRESs$value,1)-head(tail(series$IBCRESs$value, 2), 1) >= 0) {"fas fa-caret-up"} else {"fas fa-caret-down"},
        ...
        boxPlus(
          ...
          footer = fluidRow(
            column(
              ...
              descriptionBlock(
                number = paste(round(
                  tail(series$VarejoES$value,1)-head(tail(series$VarejoES$value, 2), 1), 2), "pts"), 
                numberColor = if(tail(series$VarejoES$value,1)-head(tail(series$VarejoES$value, 2), 1) >= 0) {"green"} else {"red"}, 
                numberIcon = if(tail(series$VarejoES$value,1)-head(tail(series$VarejoES$value, 2), 1) >= 0) {"fas fa-caret-up"} else {"fas fa-caret-down"},
              ...
              descriptionBlock(
                number = paste(round(
                  tail(series$ServicosES$value,1)-head(tail(series$ServicosES$value, 2), 1), 2), "pts"), 
                numberColor = if(tail(series$ServicosES$value,1)-head(tail(series$ServicosES$value, 2), 1) >= 0) {"green"} else {"red"}, 
                numberIcon = if(tail(series$ServicosES$value,1)-head(tail(series$ServicosES$value, 2), 1) >= 0) {"fas fa-caret-up"} else {"fas fa-caret-down"},
            ...
            column(
              ...
              descriptionBlock(
                number = paste("US$",round(
                  (tail(series$ExpES$value, 1) - head(tail(series$ExpES$value, 2), 1))/1000, 2), "mi"), 
                numberColor = if(tail(series$ExpES$value, 1) - head(tail(series$ExpES$value, 2), 1) >= 0) {"green"} else {"red"}, 
                numberIcon = if(tail(series$ExpES$value, 1) - head(tail(series$ExpES$value, 2), 1) >= 0) {"fa fa-caret-up"} else {"fa fa-caret-down"},
...
mod_atividade_economica_es_server <- function(input, output, session){
  ...
  output$plot2 = renderPlotly({
    plot_ly(data = series$PIBES, x = ~date, y = ~value,
            type = "scatter", mode = "lines", name = "PIB ES") %>%
      layout(title = "", xaxis = list(title = ""), yaxis = list(title = "PIB (em R$)"),
             legend = list(orientation = 'h',
                           x = 0.5,
                           xanchor = "center")
      )
  ...
  output$plot3 = renderPlotly({
    plot_ly(data = series$IBCRES[match(series$IBCRESs$date, series$IBCRES$date),],
            x = ~date, y = ~value,
            type = "scatter", mode = "lines", name = "IBCR-ES") %>%
      add_trace(y = series$IBCRESs$value, name = "IBC-ES dessazonalizado") %>%
      layout(title = "", xaxis = list(title = ""), yaxis = list(title = "IBCR-ES"),
             legend = list(orientation = 'h',
                           x = 0.5,
                           xanchor = "center")
      )
  ...
  output$plot4 = renderPlotly({
    plot_ly(data = series$VarejoES[match(series$ServicosES$date, series$VarejoES$date),],
            x = ~date, y = ~value,
